...
private:
  ...
  string scope{
      cpr::util::urlEncode("User.Read Files.ReadWrite.All offline_access")};
...
public:
  graph_token() {
    auth_url_suffix = (boost::format("/oauth2/v2.0/"
                                     "authorize?clientid=%1&response_type=code&"
                                     "scope=%2&redirect_uri=%3") %
                       APP_CLIENT_ID % scope % redirect_url)
                          .str();
  }
  void authorize_app(std::chrono::system_clock::time_point now) {
    ...
    // if url parsing fails, e.g. if user made an error copying/pasting the
    // response URI keep re-asking the user for a correct response URI instead
    // of bailing out
    ...
    cpr::Response token_r = cpr::Post(
        cpr::Url{token_url}, cpr::Payload{{"client_id", APP_CLIENT_ID},
                                          {"code", code},
    ...
    // parse the response
    simdjson::ondemand::document token_response =
        json_parser.iterate(token_response_plain);

    int64_t expires_in = token_response["expires_in"].get_int64();

    this->access_token =
        string(token_response["access_token"].get_string().value());
    this->refresh_token =
        string(token_response["refresh_token"].get_string().value());
  ...
  void renew_token(std::chrono::system_clock::time_point now) {
    ...
    cpr::Response token_r =
        cpr::Post(cpr::Url{refresh_url},
                  cpr::Payload{{"client_id", APP_CLIENT_ID},
  ...
  std::string export_session() {
    ...
    yyjson_mut_obj_add_str(doc, root, "refresh_token",
                           this->refresh_token.c_str());
    yyjson_mut_obj_add_str(doc, root, "base_url", this->base_url.c_str());
    auto refresh_token_expiry_since_epoch =
        std::chrono::duration_cast<std::chrono::seconds>(
            this->refresh_token_expiry_time.time_since_epoch())
            .count();
    yyjson_mut_obj_add_int(doc, root, "refresh_token_expiry",
                           refresh_token_expiry_since_epoch);

    string exported_session_json{yyjson_mut_write(
        doc, YYJSON_WRITE_PRETTY | YYJSON_WRITE_ESCAPE_UNICODE, NULL)};
  ...
  void restore_session(string session_data) {
    ...
    int64_t refresh_token_expiry =
        session_doc["refresh_token_expiry"].get_int64();
    refresh_token_expiry_time = std::chrono::system_clock::time_point(
